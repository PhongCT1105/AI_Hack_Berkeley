...
const STEPS = [
  ...
  { label: "Browserbase searches the live web", icon: Search },
  { label: "Shield collects and scores sources", icon: ShieldCheck },
  { label: "Claude writes from validated evidence", icon: Sparkles },
];

export default function DemoPage() {
  const [prompt, setPrompt] = useState("What is today's Bitcoin price, and what trustworthy sources support it?");
  ...
  return <div className="mx-auto max-w-5xl px-4 py-7 sm:px-6 sm:py-10">
    <section className="mb-8 max-w-2xl">
      ...
      <p className="mt-2 text-sm leading-6 text-muted-foreground">Claude plans the investigation and synthesizes the answer. Browserbase gathers live sources. Shield validates every source before it is used.</p>
    </section>
    ...
    <Card className="glass-panel">
      <CardContent className="pt-5">
        ...
        <textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} rows={4} placeholder="For example: What is today's Bitcoin price?" className="w-full resize-none rounded border border-border bg-card p-3 text-sm leading-6 outline-none focus:border-primary focus:ring-2 focus:ring-primary/15" />
