...
type CompressionMethod = "semantic_ir" | "sentence_selector";
...
export default function CompressPage() {
  ...
  const [method, setMethod] = useState<CompressionMethod>("semantic_ir");
  ...
  async function runCompression() {
    ...
    const apiBases =
      API_BASE === "http://localhost:8000" ? [API_BASE, LOCAL_API_FALLBACK] : [API_BASE];
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl">
              ...
              <h1 className="mt-2 text-3xl font-semibold tracking-normal text-zinc-950 sm:text-4xl">
                Finance source context to credibility capsule
              </h1>
              <p className="mt-3 max-w-2xl text-base leading-7 text-zinc-600">
                Domain-aware compression for finance AI agents, tuned for token
                savings and preserved source-trust facts.
              </p>
            </div>

            <div className="flex rounded-lg border border-zinc-200 bg-zinc-50 p-1">
              {(["semantic_ir", "sentence_selector"] as CompressionMethod[]).map((item) => (
                <button
                  key={item}
                  ...
                  type="button"
                >
                  {item === "semantic_ir" ? "Semantic IR" : "Sentence selector"}
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="grid gap-5">
          <div className="grid gap-4 lg:grid-cols-2">
            <label className="grid gap-2">
              ...
              <textarea
                className="min-h-[320px] resize-none rounded-lg border border-zinc-200 bg-white p-4 font-mono text-sm leading-6 outline-none ring-emerald-600/20 focus:ring-4"
                value={text}
            ...
            <div className="grid gap-2">
              <div className="flex items-center justify-between">
                ...
                <Button onClick={runCompression} disabled={loading} size="lg">
                  {loading ? <Loader2 className="animate-spin" /> : <ArrowRight />}
                  Compress
                </Button>
              </div>
              <pre className="min-h-[320px] overflow-auto rounded-lg border border-zinc-200 bg-zinc-950 p-4 font-mono text-sm leading-6 text-zinc-50">
                {result?.compressed_text || "Capsule pending."}
              </pre>
            </div>
          </div>
          ...
          <label className="grid gap-2">
            ...
            <input
              className="h-11 rounded-lg border border-zinc-200 bg-white px-3 text-sm outline-none ring-emerald-600/20 focus:ring-4"
              value={query}
          ...
          {result?.reconstructed_prompt ? (
            <div className="grid gap-2">
              <span className="text-sm font-medium text-zinc-700">Agent reconstruction</span>
              <pre className="max-h-[260px] overflow-auto rounded-lg border border-zinc-200 bg-white p-4 font-mono text-sm leading-6 text-zinc-800">
                {result.reconstructed_prompt}
              </pre>
            </div>
          ) : null}
        ...
        <aside className="grid content-start gap-4">
          <div className="rounded-lg border border-zinc-200 bg-white p-4">
            ...
            <p className="mt-2 text-4xl font-semibold text-zinc-950">
              {result ? `${result.token_savings_percent}%` : "--"}
            </p>
            <div className="mt-4 grid grid-cols-2 gap-3 border-t border-zinc-100 pt-4 text-sm">
              <Metric label="Original" value={result?.original_tokens ?? 0} />
              <Metric label="Compressed" value={result?.compressed_tokens ?? 0} />
            </div>
          </div>
          ...
          <div className="rounded-lg border border-zinc-200 bg-white p-4">
            ...
            <p className="mt-2 text-4xl font-semibold text-zinc-950">
              {result?.preservation_score != null
                ? `${Math.round(result.preservation_score * 100)}%`
                : "--"}
            </p>
            <div className="mt-4 flex flex-wrap gap-2">
              {requiredFacts.map((fact) => {
                ...
                return (
                  ...
                  >
                    {preserved ? <Check className="size-3" /> : null}
                    {fact}
                  </span>
                );
              })}
